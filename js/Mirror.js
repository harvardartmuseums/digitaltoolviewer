...
THREE.Mirror = function ( width, height, options ) {
	...
	scope.layers.set(mirrorLayer);
	scope.layer = mirrorLayer;
	mirrorLayer++;
	if (mirrorLayer >= 30) {
		mirrorLayer = 10;
	}
	...
	var mirrorShader = {
		...
		fragmentShader: [
			...
			'	color = vec4(blendOverlay(mirrorColor.r, color.r), blendOverlay(mirrorColor.g, color.g), blendOverlay(mirrorColor.b, color.b), 0.2);',
			'	gl_FragColor = color;',
			'}'
		].join( '\n' )
